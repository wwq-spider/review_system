 ...
 * @param addurl//目标页面地址
 */
function addNoSave(title,addurl,gname,width,height) {
	gridname=gname;
	createwindowNoSave(title, addurl,width,height);
}
 ...
 * @param saveurl
 */
function createwindowNoSave(title, addurl,width,height) {
 ...
 * @param addurl//目标页面地址
 */
function pubOrSave(title,addurl,gname,width,height) {
	gridname=gname;
	createwindowPubOrSave(title, addurl,width,height);
}

function pubOrUpd(title,url, id,width,height) {
	gridname=id;
	...
	createwindowPubOrSave(title, url, width, height);
}
 ...
 * @param saveurl
 */
function createwindowOnlyPub(title, addurl,width,height) {
	width = width?width:700;
	height = height?height:400;
	if(width=="100%" || height=="100%"){
		width = window.top.document.body.offsetWidth;
 ...
 * @param id//主键字段
 */
function updateReviewUser(title,url, id,width,height) {
	gridname=id;
	var rowsData = $('#'+id).datagrid('getSelections');
	if (!rowsData || rowsData.length==0) {
		tip('请选择编辑项目');
...
//发布
function publish(url,gridName,name) {
	
	gridname=gridName;
	createdialog('发布确认 ', '确定'+name+'吗 ?', url,gridName);
}

//推荐
function recomm(url,gridName,name) {
	
	gridname=gridName;
	createdialog('推荐确认 ', '确定'+name+'吗 ?', url,gridName);
}
 ...
 * @param saveurl
 */
function openuploadwin1(title, url,name,width, height) {
	gridname=name;
	$.dialog({
	    content: 'url:'+url,
	    cache:false,
 ...
 * @param gridname
 */
function saveOrPub(type) {
	var url = iframe.document.getElementById("addForm").action;

	iframe.document.getElementById("addForm").action =url +"&status="+type;
	
	//alert(iframe.document.getElementById("addForm").action);
	$('#btn_sub', iframe.document).click();
...
function doSubmit(url,name,data) {
	...
	paramsData["CSRFToken"] = $("#CSRFToken").val();
