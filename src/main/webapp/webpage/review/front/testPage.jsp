...
<script language="javascript">
	...
	$(function(){
		...
		window.localStorage.setItem("curReviewClassId", "${question.classId}")
		$(window).resize();
				...
				if (data) {
					if(data.result == 1) { //提交成功 进入下一个测评量表
						...
						if (nextClassId && nextClassId != "") {
							window.localStorage.setItem("curReviewClassId", nextClassId)
							alert("提交成功，即将进入下一个测评量表")
							window.location.href = rootPath + "/reviewFront.do?toGuide&classId=" + nextClassId + "&projectId=" + data.projectId
						} else {
							window.localStorage.removeItem("curReviewClassId")
