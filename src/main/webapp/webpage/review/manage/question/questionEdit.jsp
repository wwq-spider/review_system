...
<form id="addForm" action= "review.do?update" method="post" enctype="multipart/form-data">
	<input type="hidden" id="btn_sub" class="btn_sub" onclick="addQuestion();"/>
	<input type="hidden" id="selectCount" name="selectCount"/>
	<input type="hidden" id="delSelectIds" name="delSelectIds"/>
			...
			<td class="value" id="selectTd">
				<table style="width: 100%;" >
					...
					<c:forEach items="${answersList }" var="answer" varStatus="i">
						<tr>
							<td align="center">
								<input type="hidden" name="selectList[${i.index }].selectId" id="selectList[${i.index }].selectId" value="${answer.answerId }"></input>
								<input type="hidden" name="selectList[${i.index }].selCode" id="selectList[${i.index }].selCode" value="${answer.selCode }"></input>
								<input type="radio" name="rightAnswer" id="rightAnswer" value="${answer.selCode }"/>${answer.selCode }
							</td>
							<td align="center">
								<input id="selectList[${i.index }].selectContent" name="selectList[${i.index }].selectContent" value="${answer.selectContent }" dataType="*" type="text" style="width: 300px;" class="inputxt">
								<span class="Validform_checktip"></span><label class="Validform_label" style="display: none;">内容</label></td>
							</td>
							<td align="center">
								<input  id="selectList[${i.index }].selectGrade" name="selectList[${i.index }].selectGrade" value="${answer.selectGrade }" dataType="n" type="number" style="width: 50px;" class="inputxt">
...
//                 img.style.marginLeft = rect.left+'px';
		...
		function del() {
			var tr = $("#selectTable tr");
			var delId = document.getElementById("selectList["+(tr.length - 1)+"].selectId").value;
			var delSelectId = $("#delSelectIds").val();
			if($.trim(delSelectId) == "") {
				$("#delSelectIds").val(delId);
			} else {
				$("#delSelectIds").val(delSelectId+","+delId);
			}
